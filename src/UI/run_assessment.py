...
class AssessmentCrew:
  ...
  def run(self):
    ...
    grades_source = ExcelKnowledgeSource(
       file_paths=["grades_101.xlsx", "grades_103.xlsx"]
    )
    ...
    assignment_to_course_outcomes_source = ExcelKnowledgeSource(
       file_paths=[ "grades_101.xlsx", "COMP-101.xlsx", "grades_103.xlsx", "COMP-103.xlsx"]
    )
    course_outcomes_agent = CourseOutcomesAgent(llm=gpt_4o_high_tokens, knowledge_sources=[comp_101, comp_103]
    )
