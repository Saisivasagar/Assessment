...
for course in config['courses']:
    ...
    for section_data in course['sections']:
        ...
        excel_file = os.path.join(config['output']['excel_folder'], 
                                  f"{course_name}_{semester}_{section}_outcomes.xlsx")
        db_file = os.path.join(config['output']['database_folder'], 
                               f"{course_name}_{semester}_{section}_outcomes.db")

        acat.save_to_excel(student_outcomes, excel_file)
        acat.save_to_sqlite(db_file, student_outcomes)
